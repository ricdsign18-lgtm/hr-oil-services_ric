...
const VistaFacturaNomina = ({
  ...
  const calcularPeriodoPago = (item) => {
    ...
    const currentDateStr =
      item.fechaPago || new Date().toISOString().slice(0, 10); // Fallback
    ...
    if (emp.frecuenciaPago === "Semanal" || !emp.frecuenciaPago) {
      // Default to weekly logic if missing
    ...
    <div className="vista-factura-nomina">
      ...
      <div className="factura-header">
        <div className="factura-title">
          <h5>
            {title ||
              (isPersonal ? "Nómina de Personal" : "Pagos a Contratistas")}
          </h5>
        ...
        <div className="factura-meta">
          <div>
            <strong>Tasa de Cambio:</strong> {parseFloat(tasaCambio).toFixed(4)}{" "}
            Bs/$
          </div>
        </div>
      </div>
            ...
            {data.map((item, idx) => {
              if (isPersonal) {
                ...
                return (
                  <tr key={emp.id || idx}>
                    ...
                    <td className="text-right">
                      ${(item.montoDiarioCalculado || 0).toFixed(2)}
                    </td>
                    <td className="text-center">
                      {item.horasExtras?.diurna || 0}
                    </td>
                    <td className="text-center">
                      {item.horasExtras?.nocturna || 0}
                    </td>
                    <td className="text-right">
                      ${(item.totalHorasExtrasUSD || 0).toFixed(2)}
                    </td>
                    <td className="text-right text-red">
                      ${(item.deduccionesManualesUSD || 0).toFixed(2)}
                    </td>
                    <td className="text-right text-red">
                      ${(item.adelantosUSD || 0).toFixed(2)}
                    </td>
                    <td className="text-right">
                      <strong>${(item.subtotalUSD || 0).toFixed(2)}</strong>
                    </td>
                    <td className="text-right">
                      Bs {(item.montoExtraBs || 0).toFixed(2)}
                    </td>
                    <td className="text-right">
                      ${(item.montoExtraUSD || 0).toFixed(2)}
                    </td>
                    <td className="text-right highlight-blue">
                      <strong>${(item.montoTotalUSD || 0).toFixed(2)}</strong>
                    </td>
                    <td className="text-right">
                      {parseFloat(tasaCambio).toFixed(4)}
                    </td>
                    <td className="text-right highlight-bs">
                      <strong>Bs {(item.totalPagarBs || 0).toFixed(2)}</strong>
                    </td>
                    <td>{item.bancoPago || "-"}</td>
                    {/* <td>{calcularPeriodoPago(item)}</td> */}
                    <td>-</td>{" "}
                    {/* Contrato hard to access without context, passing placeholder */}
                    <td className="text-small">{item.observaciones}</td>
                    {showLegalDeductions && (
                      <>
                        <td className="text-center">
                          {esAdministrativo
                            ? (emp.porcentajeIslr || "0") + "%"
                            : "-"}
                        </td>
                        <td className="text-right">
                          {esAdministrativo
                            ? item.desgloseDeduccionesLey?.ivss?.toFixed(2) ||
                              "0.00"
                            : "-"}
                        </td>
                        <td className="text-right">
                          {esAdministrativo
                            ? item.desgloseDeduccionesLey?.paroForzoso?.toFixed(
                                2,
                              ) || "0.00"
                            : "-"}
                        </td>
                        <td className="text-right">
                          {esAdministrativo
                            ? item.desgloseDeduccionesLey?.faov?.toFixed(2) ||
                              "0.00"
                            : "-"}
                        </td>
                        <td className="text-right">
                          {esAdministrativo
                            ? item.desgloseDeduccionesLey?.islr?.toFixed(2) ||
                              "0.00"
                            : "-"}
                        </td>
                        <td className="text-right">
                          <strong>
                            {esAdministrativo
                              ? item.deduccionesLeyBs?.toFixed(2) || "0.00"
                              : "-"}
                          </strong>
                        </td>
                      </>
                ...
                if (item.dias_trabajados_detalle) {
                  count = Object.values(item.dias_trabajados_detalle).filter(
                    (v) => Number(v) > 0,
                  ).length;
                } else {
                ...
                const totalBs =
                  item.monto_total_bs || totalUSD * tasaCambio || 0;
                ...
                return (
                  <tr key={item.id || idx}>
                    ...
                    <td className="text-right highlight-blue">
                      <strong>${Number(totalUSD).toFixed(2)}</strong>
                    </td>
                    <td className="text-right">
                      {parseFloat(tasaCambio).toFixed(4)}
                    </td>
                    <td className="text-right highlight-bs">
                      <strong>Bs {Number(totalBs).toFixed(2)}</strong>
                    </td>
      ...
      <div className="factura-footer">
        <div className="factura-actions">
          <button
            className="btn-factura-action btn-factura-edit"
            onClick={onEdit}
          >
            Editar
          </button>
          <button
            className="btn-factura-action btn-factura-delete"
            onClick={onDelete}
          >
