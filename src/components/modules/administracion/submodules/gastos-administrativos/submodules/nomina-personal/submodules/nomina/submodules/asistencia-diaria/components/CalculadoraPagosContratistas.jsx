...
const CalculadoraPagosContratistas = ({
  projectId,
  fechaPago,
  tasaCambio,
  onGuardar,
  ...
  const [montoExtra, setMontoExtra] = useState({}); // { cid: amount }
      ...
      try {
        ...
        if (initialData && initialData.pagos) {
          ...
          (data || []).forEach((record) => {
            ...
            agg[cid][dayKey] =
              (agg[cid][dayKey] || 0) +
              (record.cantidad_personal_asistente || 0);
          });
  ...
  const calculateTotalUSD = (c) => {
    const base =
      calculateTotalPersonnelDays(c.id) * (parseFloat(c.monto_diario) || 0);
    const extra = parseFloat(montoExtra[c.id] || 0);
    return base + extra;
  };
  ...
  const handleGuardar = async () => {
    // Validate
    if (!fechaPago) {
      showToast("Falta fecha pago", "warning");
      return;
    }
    if (!tasaCambio || parseFloat(tasaCambio) <= 0) {
      showToast("Tasa de cambio inválida", "warning");
      return;
    }
    ...
    <div className="calculadora-contratistas">
      ...
      <div className="table-responsive">
        <table className="contratistas-pay-table">
          ...
          <tfoot>
            <tr>
              <td colSpan="4" className="text-right label-total">
                TOTALES:
              </td>
