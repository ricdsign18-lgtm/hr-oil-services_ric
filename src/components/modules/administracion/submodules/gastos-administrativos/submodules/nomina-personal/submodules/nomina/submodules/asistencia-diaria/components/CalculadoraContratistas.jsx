...
const CalculadoraContratistas = ({
  ...
  const [montoExtra, setMontoExtra] = useState({}); // New state for Monto Extra
  ...
  const daysOrder = visibleDay
    ? allDaysOrder.filter((d) => d === visibleDay)
    : allDaysOrder;
  ...
  useEffect(() => {
    const fetchDailyAttendance = async () => {
      ...
      // Only fetch if we are in Payroll mode (showAmounts=true), have a project, date...
      // BUT if we are editing (initialData exists), we usually skip fetching.
      // EXCEPTION: If initialData is "legacy" (has 0s in details because they weren't saved),
      // we SHOULD fetch attendance to backfill the zeros.
      const hasSavedDetails = initialData?.pagos?.some(
        (p) =>
          p.dias_trabajados_detalle &&
          Object.values(p.dias_trabajados_detalle).some((v) => v > 0),
      );

      if ((!showAmounts && !visibleDay) || !projectId || !fechaPago) return;

      // If we have initialData AND valid saved details, trust the snapshot.
      if (initialData && hasSavedDetails) return;
      ...
      try {
        ...
        if (visibleDay) {
          ...
          console.log(
            "Fetching Contractor Attendance for Range:",
            startDateStr,
            "to",
            endDateStr,
          );
    ...
    if (initialData && initialData.pagos && contratistas.length > 0) {
      ...
      initialData.pagos.forEach((p) => {
        ...
        if (p.contratista_id) {
          ...
          // Load existing monto_extra if available
          if (p.monto_extra) {
            newMontoExtra[p.contratista_id] = p.monto_extra;
          }
        }
      });
  ...
  const handleDayChange = (id, day, value) => {
    ...
    if (numValue > maxLimit) {
      showToast(
        `El límite de personal para ${contractor?.nombre_contratista || "este contratista"} es ${maxLimit}`,
        "error",
      );
  ...
  const calculateTotalUSD = (contratista) => {
    ...
    const base =
      totalPersonnelDays * (parseFloat(contratista.monto_diario) || 0);
    const extra = parseFloat(montoExtra[contratista.id] || 0);
    return base + extra;
  };
  ...
  const handleGuardar = async () => {
    ...
    if (
      !fechaPago ||
      (showAmounts && (!tasaCambio || parseFloat(tasaCambio) <= 0))
    ) {
      if (!fechaPago) {
      ...
      if (showAmounts && (!tasaCambio || parseFloat(tasaCambio) <= 0)) {
        showToast(
          "Por favor ingrese una tasa de cambio válida para calcular los pagos en Bolívares",
          "warning",
        );
        return;
      }
    ...
    if (onCalcular) {
      const pagosDetalle = contratistas
        .map((c) => {
          ...
          return {
            ...
            monto_extra: parseFloat(montoExtra[c.id] || 0),
    ...
    if (visibleDay) {
      ...
      console.log(
        "Saving to asistencia_contratistas... Payload:",
        upsertPayload,
      );
      ...
      try {
        const { error } = await supabase
          ...
          .upsert(upsertPayload, {
            onConflict: "project_id, contratista_id, fecha",
          });
      ...
      console.log(
        "Updating Payment Snapshot (pagos_contratistas)... Payload:",
        payload,
      );
      ...
      try {
        ...
        if (!idToUpdate) {
          ...
          showToast(
            "Error: No se identificó el registro para actualizar.",
            "error",
          );
          return;
        }
  ...
  const totalGeneralUSD = contratistas.reduce(
    (acc, c) => acc + calculateTotalUSD(c),
    0,
  );
  ...
  return (
    <div className="calculadora-contratistas">
      <h3>
        {visibleDay ? "Asistencia de Contratistas" : "Nómina de Contratistas"}
      </h3>
      ...
      <div className="table-responsive">
        <table className="contratistas-pay-table">
          <thead>
            <tr>
              ...
              <th className="text-center">
                {showAmounts
                  ? "Detalle Asistencia (Personas x Día)"
                  : "Personal por Día"}
              </th>
              {showAmounts && (
                <>
                  <th className="text-right">Monto Diario ($)</th>
                  <th className="text-right">Monto Extra ($)</th>
              ...
              <tr key={c.id}>
                <td>
                  ...
                  <div className="c-meta">
                    Max Personal: {c.cantidad_personal}
                  </div>
                </td>
                <td>
                  {showAmounts ? (
                      ...
                      {daysOrder.map((day) => {
                        ...
                        return (
                          <div key={day} className="day-val-item">
                            <span className="day-label-sm">
                              {dayLabels[day]}
                            </span>
                            <span
                              className={`day-val-num ${val > 0 ? "has-val" : ""}`}
                            >
                              {val}
                            </span>
                          </div>
                    ...
                    <div className="days-inputs">
                      {daysOrder.map((day) => {
                        ...
                        return (
                          <div
                            key={day}
                            className={`day-input-group ${visibleDay ? "single-day-mode" : ""}`}
                          >
                            {!visibleDay && (
                            ...
                            <div className="check-input-stack">
                              ...
                              <input
                                ...
                                value={
                                  isChecked && currentVal > 0 ? currentVal : ""
                                }
                                placeholder="0"
                                onChange={(e) =>
                                  handleDayChange(c.id, day, e.target.value)
                                }
                                min="0"
                ...
                {showAmounts && (
                  <>
                    ...
                    <td className="text-right">
                      $ {parseFloat(c.monto_diario).toFixed(2)}
                    </td>
                    <td className="text-right">
                      <input
                        type="number"
                        className="monto-extra-input"
                        value={montoExtra[c.id] || ""}
                        placeholder="0"
                        min="0"
                        onChange={(e) =>
                          setMontoExtra((prev) => ({
                    ...
                    <td>
                      <select
                        ...
                        onChange={(e) =>
                          handleBancoChange(c.id, e.target.value)
                        }
                      >
          ...
          {showAmounts && (
            <tfoot>
              <tr>
                <td colSpan="3" className="text-right label-total">
                  TOTALES:
                </td>
                <td className="text-right val-total">
        ...
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <div className="modal-actions">
              ...
              <button className="confirm-btn" onClick={handleAddBanco}>
                Agregar
              </button>
            </div>
          </div>
