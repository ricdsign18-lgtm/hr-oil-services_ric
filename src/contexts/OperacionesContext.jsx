...
export const OperacionesProvider = ({ children }) => {
  ...
  const getRequerimientos = useCallback(async () => {
    ...
    if (!isOperationsRole && userRoleConfig?.scope !== "*") {
    ...
    if (error) {
      ...
      if (error.code === "42703") {
        ...
        console.error(
          "Error: Columna 'user_id' no encontrada. La visibilidad jerárquica no funcionará sin este campo.",
        );
      }
    } else {
  ...
  const addRequerimientoItem = useCallback(
    async (itemData) => {
      setLoading(true);
      ...
      const { error } = await supabase.from("requerimiento_items").insert([
        {
          ...itemData,
  ...
  const addRequerimiento = useCallback(
    async (requerimientoData) => {
      if (!selectedProject || !user) return;
      setLoading(true);
  ...
  const updateRequerimientoItem = useCallback(
    async (itemId, updatedData) => {
      ...
      if (error) {
        ...
        console.error(
          "Error updating item: No se actualizó ninguna fila. RLS Bloqueando.",
        );
        showToast(
          "Error update: No tienes permiso para editar este item.",
          "error",
        );
      } else {
  ...
  const approveRequerimiento = useCallback(
    async (reqId) => {
      setLoading(true);
  ...
  const approveRequerimientoItem = useCallback(
    async (itemId) => {
      setLoading(true);
      ...
      if (error) {
        ...
        console.error(
          "Error approving item: No se actualizó ninguna fila. Posible bloqueo RLS.",
        );
        showToast(
          "Error update: No tienes permiso para aprobar este item.",
          "error",
        );
      } else {
        ...
        if (parentId) {
          ...
          if (!countError && count === 0) {
            console.log(
              "Todos los items aprobados. Actualizando requerimiento padre a pendiente.",
            );
            ...
            if (parentError)
              console.error("Error syncing parent status:", parentError);
          }
        }
  ...
  const rejectRequerimientoItem = useCallback(
    async (itemId) => {
      setLoading(true);
  ...
  const cancelRequerimientoItem = useCallback(
    async (itemId) => {
      setLoading(true);
      const { error } = await supabase
        .from("requerimiento_items")
  ...
  const deleteRequerimientoItem = useCallback(
    async (itemId) => {
      setLoading(true);
      const { error } = await supabase
        .from("requerimiento_items")
  ...
  const cancelRequerimiento = useCallback(
    async (reqId) => {
      if (!window.confirm("¿Estás seguro de cancelar toda esta solicitud?"))
        return;
  ...
  const withdrawInventory = useCallback(
    async (withdrawalData) => {
      if (!selectedProject) return;
      setLoading(true);
      const { inventario_id, cantidad_retirada, retirado_por, observaciones } =
        withdrawalData;
      ...
      const isNonConsumable = ["HERRAMIENTA", "EQUIPO"].includes(
        item.categoria_producto?.toUpperCase(),
      );
        ...
        .insert([
          {
            ...
            retirado_por,
            observaciones:
              observaciones || "Retiro registrado desde Operaciones",
  ...
  const addPurchase = useCallback(
    async (purchaseData) => {
      ...
      // Calcular totales
      const iva = purchaseData.aplica_iva
        ? parseFloat(purchaseData.precio_unitario) *
      ...
      const precio_unitario_usd_aprox =
        parseFloat(total_usd) / parseFloat(purchaseData.cantidad);

      const { error: upsertError } = await supabase.from("productos").upsert(
        {
          nombre_producto,
          categoria_producto,
          unidad,
      ...
      if (existingItem) {
        const newQuantity =
          Number(existingItem.cantidad_disponible) +
          Number(purchaseData.cantidad);
        const { error: updateError } = await supabase
          .from("inventario")
          .update({
      ...
      const { data: pendingReqItems, error: reqItemsError } = await supabase
        .from("requerimiento_items")
        .select(
          "id, cantidad_requerida, cantidad_comprada, requerimiento_id, status",
        )
        .eq("nombre_producto", purchaseData.nombre_producto)
      ...
      if (reqItemsError) {
        console.error(
          "Error fetching pending requirement items:",
          reqItemsError,
        );
        ...
        for (const reqItem of pendingReqItems) {
          ...
          const remainingToBuy =
            reqItem.cantidad_requerida - reqItem.cantidad_comprada;
          const purchasedAmount = Math.min(remainingQuantity, remainingToBuy);

          if (purchasedAmount > 0) {
            const newCantidadComprada =
              reqItem.cantidad_comprada + purchasedAmount;
            ...
            if (updateReqItemError) {
              console.error(
                "Error updating requirement item:",
                updateReqItemError,
              );
            }
